...
partes_html = [
    ...
    ".screen { width: 100%; height: 100%; position: absolute; top: 0; left: 0; display: none; background-size: 100% 100%; background-repeat: no-repeat; }",
    ".active { display: block; }",
    "#menu-screen { background-image: url('", img_capa, "'); display: flex; flex-direction: column; align-items: center; justify-content: center; }",
    "#start-btn { padding: 15px 50px; font-family: 'MedievalSharp', cursive; font-size: 26px; font-weight: bold; cursor: pointer; color: #fff; background: linear-gradient(135deg, #4a0000 0%, #1a0000 100%); border: 2px solid #8b0000; border-radius: 4px; box-shadow: 0 0 15px rgba(139, 0, 0, 0.6); text-shadow: 2px 2px 4px #000; transition: all 0.3s; margin-top: 200px; }",
    ...
    "#player { width: 70px; height: 100px; background-image: url('", img_player, "'); background-size: contain; background-repeat: no-repeat; background-position: bottom; position: absolute; bottom: 55px; left: 100px; z-index: 10; }",
    ".enemy { width: 65px; height: 95px; background-image: url('", img_inimigo, "'); background-size: contain; background-repeat: no-repeat; background-position: bottom; position: absolute; bottom: 55px; z-index: 5; }",
    "#boss-sf { width: 160px; height: 220px; background-image: url('", img_boss, "'); background-size: contain; background-repeat: no-repeat; background-position: bottom; position: absolute; bottom: 55px; right: 40px; display: none; z-index: 6; filter: drop-shadow(0 0 15px rgba(255, 0, 0, 0.5)); }",
    ...
    "</style></head><body>",
    "<div id='game-container'><div id='menu-screen' class='screen active'><button id='start-btn' onclick='startGame()'>INICIAR JOGO</button></div>",
    ...
    "if (currentStage < totalStages) { bossSf.style.display = 'none'; createEnemy(400, 2.5); createEnemy(650, -2.5); } else { stageIndicator.innerText = 'CENÁRIO FINAL: BOSS SF'; bossSf.style.display = 'block'; } }",
    "function createEnemy(startX, speed) { const enemyEl = document.createElement('div'); enemyEl.className = 'enemy'; enemyEl.style.left = startX + 'px'; enemiesContainer.appendChild(enemyEl); enemiesList.push({ element: enemyEl, x: startX, speed: speed, minX: startX - 120, maxX: startX + 120 }); }",
    ...
    "if (Math.abs(playerX - enemy.x) < 50 && playerY < 120) { if (isAttacking) { enemy.element.remove(); enemiesList = enemiesList.filter(e => e !== enemy); } else if (!isJumping) { playerX = 50; player.style.left = playerX + 'px'; } } });",
    ...
    "switch(event.code) { case 'KeyA': case 'ArrowLeft': movePlayer(-25); break; case 'KeyD': case 'ArrowRight': movePlayer Shivaji = 25; movePlayer(25); break; case 'KeyW': case 'ArrowUp': jumpPlayer(); break; case 'Space': attackPlayer(); break; } });",
    "</script></body></html>"
]
